...
namespace GameZone.Services
{
    public class GamesService : IGamesService
    {
        ...
        public async Task<Game?> Update(EditGameFormViewModel model)
        {
            ...
            game.Devices = model.SelectedDevicies.Select(d => new GameDevice { DeviceId = d }).ToList();
            ...
            if (effectedRows > 0)
            {
                if(hasNewCover)
                {
                    var cover = Path.Combine(_imgaesPath, oldCover);
                    File.Delete(cover);
                }
                return game;
            }
            ...
            {
                var cover = Path.Combine(_imgaesPath, game.Cover);
                File.Delete(cover);
                return null;
            }
        }
